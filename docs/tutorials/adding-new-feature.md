...
本教學將引導您如何在 Go Template 專案中新增一個新的功能模組。我們將以新增一個
"產品" (Product) 功能為例。
...
3. **定義業務邏輯 (`internal/services/product.go` 和
...
4. **編寫處理函數 (`internal/api/handlers/product.go`)**：
    ...
    func (h *Product) CreateProduct(c *gin.Context) {
        ...
        response.SuccessResponse(c, http.StatusCreated,
...
5. **定義路由 (`internal/api/routes/product.go`)**：
    ...
    func SetupProduct(r *gin.Engine, Product *handlers.Product,
        ...
        products := r.Group("/products")
        {
            ...
            // 需要身份驗證
            products.PUT("/:id", authMiddleware.Handle(), Product.UpdateProduct)
            // 需要身份驗證
            products.DELETE("/:id", authMiddleware.Handle(), Product.DeleteProduct)
        }
    }
    ```
...
7. **執行 `make generate` 或在 `cmd/go-template/` 目錄下執行 `go generate ./...`
    重新產生 `cmd/go-template/wire_gen.go`**。

8. **執行資料庫遷移：** 執行 `go run migrations/migrate.go`。 (需要先在
    `migrations/migrate.go` 中加入 `db.AutoMigrate(&models.Product{})`)
